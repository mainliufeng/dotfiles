...
### 完成通知 Hook
配置了 Stop hook，可在 Claude Code 执行完成后自动发送桌面通知。

**功能特性：**
- ✅ Linux 桌面环境通过 `notify-send` 发送通知
- ✅ WSL2 环境下可回退至 Windows Toast 通知
- ✅ 显示项目名称和会话 ID
- ✅ 可选：播放系统提示音
...
**依赖要求：**
- `jq` - JSON 解析工具（必需）
- `libnotify`/`notify-send`（Linux 通知）
- `paplay` - 音频播放（可选）

Arch Linux 安装示例：
```bash
sudo pacman -S jq libnotify pulseaudio
```
