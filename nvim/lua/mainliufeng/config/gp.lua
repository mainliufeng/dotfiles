...
local config = {
    ...
    providers = {
		...
		qianxun = {
			endpoint = "https://qianxun.rcrai.com/open/qianxun/v1" .. "/chat/completions",
			secret = os.getenv("QIANXUN_ACCESS_TOKEN"),
		},
    },
	...
	agents = {
		...
		{
            provider = "qianxun",
			name = "ChatQianxun128k",
			chat = true,
			command = false,
			-- string with model name or table with model name and parameters
			model = { model = "qianxun-l-128k-nocof", temperature = 0.7 },
			-- system prompt (use this to specify the persona/role of the AI)
			system_prompt = require("gp.defaults").chat_system_prompt,
		},
		{
		...
		{
			provider = "qianxun",
			name = "CodeQianxun128k",
			chat = false,
			command = true,
			-- string with model name or table with model name and parameters
			model = { model = "qianxun-l-128k-nocof", temperature = 0.2, top_p = 1 },
			-- system prompt (use this to specify the persona/role of the AI)
			system_prompt = require("gp.defaults").code_system_prompt,
		},
    },
