...
5. Skip registry rows whose `platforms` value does not match the current platform, unless the value is `all`.
...
- If a skill is missing from both registries, stop and ask the user whether to add it.
