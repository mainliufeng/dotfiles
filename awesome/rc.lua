...
-- Awesome
    ...
    awful.key({ modkey, "Shift" }, "l", function() awful.spawn.with_shell("slock") end,
    ...
    awful.key({ modkey, }, "r", function() awful.spawn.with_shell("dwm-rofi") end,
...
function center_and_resize(c)
    -- 设定窗口的宽度和高度为屏幕的 80%
    local factor = 0.8
    local screen_geo = c.screen.workarea

    local width = screen_geo.width * factor
    local height = screen_geo.height * factor

    -- 计算居中的位置
    local x = (screen_geo.width - width) / 2
    local y = (screen_geo.height - height) / 2
...
awful.rules.rules = {
    ...
    {
        rule_any = {
            instance = {"scratch", "chat.openai.com"}
        },
        properties = {
            floating = true,
        },
        callback = center_and_resize
    },
...
awful.spawn.with_shell("pgrep picom || picom --config ~/.config/awesome/picom.conf --vsync")
