...
- **旧链接会变成 404**：重建只能改写产物里的链接，已经缓存在手机里的页面还在指着
  老路径（我们的日志里就能看到手机一直在请求 `/首页`）。`quartz-web-serve` 遇到这种
  “页面不存在但同名页面在别处” 的请求返回 302 到最浅的那个同名页；响应带
  `Cache-Control: no-cache`，下次加载会重新校验，拿到带修正链接的新页。
