...
pick_wallpaper_dir() {
  ...
  local candidates=(
    "${SWWW_WALLPAPER_DIR:-}"
    "${WALLPAPER_DIR:-}"
    # Backward compat (old name when stored under hyprland/):
    "${HYPR_WALLPAPER_DIR:-}"
...
state_dir() {
  printf '%s\n' "${SWWW_STATE_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/swww-wallpaper}"
}
...
legacy_last_wallpaper_file() {
  printf '%s\n' "$(legacy_state_dir)/last.txt"
}
...
read_last_wallpaper() {
  ...
  # Backward compat: reuse old state from `hypr-wallpaper`.
  f="$(legacy_last_wallpaper_file)"
  [[ -f "${f}" ]] || return 0
  head -n 1 "${f}" || true
}
...
main() {
  if ! command -v swww >/dev/null 2>&1; then
    echo "[wallpaper] Error: 'swww' not found. Install it first (see ~/dotfiles/linux/desktop/swww/setup.sh)." >&2
    exit 1
  fi
  ...
  if [[ -z "${dir}" ]]; then
    dir="$(pick_wallpaper_dir)" || {
      ...
      echo "[wallpaper] Tip: set SWWW_WALLPAPER_DIR or WALLPAPER_DIR to your wallpaper folder." >&2
      exit 1
    }
  fi
